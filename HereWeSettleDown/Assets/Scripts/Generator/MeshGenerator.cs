...
namespace Generator
{
    ...
    {
        public static MeshData[,] GenerateChunkMeshes(int chunkWidth, int chunkHeight, int chunkXCount, int chunkYCount, float[,] heightMap, 
            Vector2Int xRandomRange, Vector2Int yRandomRange, float heightMultiplier, AnimationCurve heightCurve, ColorPack[,] colorMap)
        {
                    ...
                    for (int x = cX * chunkWidth ; x < cX * chunkWidth  + chunkWidth ; x++)
                    {
                        for (int y = cY * chunkHeight ; y < cY * chunkHeight  + chunkHeight ; y++)
                        {
                            ...
                            for (int i = 0; i < 2; i++)
                            {
                                ...
                                // Scale vertPositions
                                for (int j = 0; j < 3; j++)
                                    vertPositions[j] = Vector3.Scale(vertPositions[j], new Vector3(1, heightMultiplier, 1));
